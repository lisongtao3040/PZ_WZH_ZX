...
class InterfaceTester:
    ...
    def test_interface(self):
        """测试目标接口"""
        print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] 开始测试接口...")
        print(f"目标URL: {self.test_url}")
        print(f"请求数据: {json.dumps(self.test_data, ensure_ascii=False, indent=2)}")
    ...
    def run_test_with_monitoring(self):
        ...
        # 执行接口测试
        test_result = self.test_interface()
...
def main():
    ...
    # 运行测试
    test_result = tester.run_test_with_monitoring()
