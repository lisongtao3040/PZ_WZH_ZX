...
Partial Class pt_v_A02_check_result_ms
    ...
    Private Sub JsLoad()
        ...
        sb.AppendLine("       ,检查开始时间:{visible:false}")
        sb.AppendLine("       ,检查终了时间:{visible:false}")
        ...
        Else
            ...
            For Each myChar In CLoginInfo.department_cd
                ...
                End If
            Next
            'sb.AppendLine("    var awaysWhere = "" AND 部门 in(" & department_cd & ") AND 检查日期='" & Now.ToString("yyyy-MM-dd") & "'"";")
            sb.AppendLine("    var awaysWhere = "" AND 部门 in(" & department_cd & ",'') "";")
        End If

        'sb.AppendLine("    var table = new dbTable(""article"", "".jqPanelTable"", "".jqPanelNumOfPages"", ""v_A02_check_result_ms"", ""检查明细表"", 1, """ & CLoginInfo.user_cd & """,""" & CLoginInfo.user_name & """, colsParams,pubInitParams, awaysWhere,500);")
        'sb.AppendLine("    table.SetColumnsWidth(table.Columns);")



        sb.AppendLine("setTimeout(function () {")
        sb.AppendLine("    var table = new dbTable(""article"", "".jqPanelTable"", "".jqPanelNumOfPages"", ""v_A02_check_result_ms"", ""检查明细表"", 1, """ & CLoginInfo.user_cd & """,""" & CLoginInfo.user_name & """, colsParams,pubInitParams, awaysWhere,500,false);")
        sb.AppendLine("    table.SetColumnsWidth(table.Columns);")

        sb.AppendLine("     $('.检查日期YMD_start').val('" & Now.AddDays(0).ToString("yyyy-MM-dd") & "');")
        sb.AppendLine("     $('.检查日期YMD_end').val('" & Now.AddDays(0).ToString("yyyy-MM-dd") & "');")
