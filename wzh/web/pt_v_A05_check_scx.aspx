<%@ Page Language="VB" AutoEventWireup="false" CodeFile="pt_v_A05_check_scx.aspx.vb" Inherits="pt_v_A05_check_scx" %>
...
<body>
    ...
    <script>
        function changeColor(that) {
            ...
            $(".trMs").each(function () {
                if (parseInt($(this).find('td').eq(4).text()) > parseInt($(this).find('td').eq(5).text())) {
                    $($(this).find('td').eq(4).find('div')).css("color", "red");
                } else {
                    //$($(this).find('td').eq(4).find('div')).css("color", "blue");
                }
            });
